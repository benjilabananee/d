...
.builder \
.master("local") \
.appName('ex6_calculate_reviews2') \
...
.option("subscribe", "gps-user-review-sourceaaaa") \
...
static_data_df = spark.read.parquet('s3a://spark/data/source/google_reviews/')
...
.format('kafka') \
.option("kafka.bootstrap.servers", "course-kafka:9092") \
.option("topic", "gps-with-reviewsaaa") \
